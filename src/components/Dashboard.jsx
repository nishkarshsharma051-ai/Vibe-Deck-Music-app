...
const MOODS = [
  ...
  { id: 'romance', label: 'Romance',  icon: 'favorite',         query: 'love romantic songs',        color: '#FA243C' },
...
/* ── Apple Music-style Shelf (12px Artwork Radius) ── */
  ...
  return (
    <section className="space-y-2.5 animate-cardReveal">
      <div className="flex items-center justify-between px-0.5">
        ...
        <button className="text-[13px] font-semibold text-[#FA243C] hover:opacity-75 transition-opacity">
          See All
        </button>
      </div>
      <div className="shelf-scroll flex gap-3.5 overflow-x-auto pb-2 hide-scrollbar -mx-1 px-1">
        {items.map((track, idx) => (
          ...
          >
            {/* Standard 12px artwork radius */}
            <div className="relative aspect-square overflow-hidden rounded-[12px] bg-[#1C1C1E] shadow-lg group-hover:shadow-xl transition-shadow duration-200">
              <img
                src={track.coverUrl}
                alt={track.title}
            ...
            <p className="truncate text-[12px]" style={{ color: 'rgba(235,235,245,0.55)' }}>{track.artist}</p>
          </button>
        ))}
      </div>
    </section>
      ...
      <div className="shelf-scroll flex gap-3.5 overflow-x-auto pb-2 hide-scrollbar -mx-1 px-1">
        {items.map((album, idx) => (
          ...
          >
            ...
            <p className="truncate text-[12px]" style={{ color: 'rgba(235,235,245,0.55)' }}>{album.artist}</p>
          </button>
        ))}
      </div>
    </section>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isActive
                  ? '0.5px solid #FA243C'
                  : '0.5px solid rgba(84, 84, 88, 0.30)',
              }}
            >
              <span
                className={`material-symbols-outlined text-[18px] ${isLoading ? 'animate-spin' : ''}`}
                style={{ color: isActive ? '#FFFFFF' : mood.color, fontVariationSettings: "'FILL' 1" }}
              >
                {isLoading ? 'autorenew' : mood.icon}
              </span>
              <span
                className="text-[11px] font-semibold tracking-wide"
                style={{ color: isActive ? '#FFFFFF' : 'rgba(235,235,245,0.75)' }}
              >
                {mood.label}
              </span>
            </button>
...
export default function Dashboard({
  ...
  /* Fluid ambient gradient driven by accentColor / artwork */
  const heroBg = accentColor
    ? `radial-gradient(ellipse 90% 60% at 50% -20%, hsl(${accentColor.h}, ${Math.min(accentColor.s, 65)}%, 18%) 0%, #000000 80%)`
    : 'radial-gradient(ellipse 90% 60% at 50% -20%, rgba(250, 36, 60, 0.15) 0%, #000000 80%)';
  ...
  return (
    ...
    >
      ...
      {/* ── Quick Picks: iOS Inset Grouped Table Style ── */}
      {quickPicks.length > 0 && (
        <section className="mb-6 animate-cardReveal">
          <div className="overflow-hidden rounded-[12px]" style={{ border: '0.5px solid rgba(84, 84, 88, 0.35)', background: 'rgba(28, 28, 30, 0.65)' }}>
            <div className="grid grid-cols-1 divide-y divide-[rgba(84,84,88,0.30)] sm:grid-cols-2 sm:divide-y-0 sm:divide-x">
              {quickPicks.map((track) => (
                ...
                >
                  ...
                  <span className="material-symbols-outlined text-[18px] text-[#FA243C] opacity-0 group-hover:opacity-100 transition-opacity">
                    play_circle
                  </span>
                </button>
              ))}
